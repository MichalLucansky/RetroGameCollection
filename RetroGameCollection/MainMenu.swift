...
class MainMenu: SKScene{
    ...
    override func didMove(to view: SKView) {
        initialization()
        Analytics.logEvent("MainMenuOpened", parameters: nil)
        GameCenterManager.instance.authentificatePlayer()
                ...
                case "Pong":
                    if let view = self.view {
                        ...
                        if let scene = PongMultiSingleSel(fileNamed: "PongMultiSingleSel") {
                            ...
                            // Present the scene
                            Analytics.logEvent("MainMenuPongGameLaunch", parameters: nil)
                ...
                case "BlockBreaker" :
                    if let view = self.view {
                        if let scene = BlockBreaker(fileNamed: "BlockBreakerScene") {
                            scene.scaleMode = .aspectFit
                            Analytics.logEvent("MainMenuBlockBReakerGameOpened", parameters: nil)
                            view.presentScene(scene,transition: SKTransition.moveIn(with: SKTransitionDirection.left, duration: TimeInterval(0.5)))
                        }
                    }
                ...
                case "SpaceInvaders" :
                    if let view = self.view {
                        if let scene = SpaceInvadersClass(fileNamed: "SpaceInvadersScene") {
                            scene.scaleMode = .aspectFit
                            Analytics.logEvent("MainMenuSpaceInvadersOpened", parameters: nil)
                            view.presentScene(scene,transition: SKTransition.moveIn(with: SKTransitionDirection.left, duration: TimeInterval(0.5)))
                        }
                    }
                ...
                case "Snake" :
                    if let view = self.view {
                        if let scene = GameScene(fileNamed: "GameScene") {
                            scene.scaleMode = .aspectFit
                            Analytics.logEvent("MainMenuSnakeOpened", parameters: nil)
                            view.presentScene(scene,transition: SKTransition.moveIn(with: SKTransitionDirection.left, duration: TimeInterval(0.5)))
                        }
                    }
                ...
                case "Settings" :
                    if let view = self.view {
                        if let scene = Settings(fileNamed: "Settings") {
                            scene.scaleMode = .aspectFit
                            Analytics.logEvent("MainMenuSettingsOpened", parameters: nil)
                            view.presentScene(scene,transition: SKTransition.moveIn(with: SKTransitionDirection.left, duration: TimeInterval(0.5)))
                        }
                    }
                ...
                case "Credits" :
                    if let view = self.view {
                        if let scene = Credits(fileNamed: "Credits") {
                            scene.scaleMode = .aspectFit
                            Analytics.logEvent("MainMenuCreditsOpened", parameters: nil)
                            view.presentScene(scene,transition: SKTransition.moveIn(with: SKTransitionDirection.left, duration: TimeInterval(0.5)))
                        }
                    }
                    
                case "Achivements" :
                    Analytics.logEvent("MainMenuAchivementsOpened", parameters: nil)
                    GameCenterManager.instance.showLeaderboard(gcDelegate: self)
...
extension MainMenu: GKGameCenterControllerDelegate {
